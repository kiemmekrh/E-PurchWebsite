...
function toDate($value) {
    if (empty($value) || $value === '' || $value === '0000-00-00') return null;
    return $value;
}
...
/**
 * Clamp gap_percent ke range DECIMAL(5,2): -999.99 sampai 999.99
 */
...
try {
    ...
    // 1. INSERT Comparison_Table (Parent)
    $stmt = $pdo->prepare("
    ...
    $stmt->execute([
        ...
        ':plan_quantity' => toNumber($data['qty_pr'] ?? 0),
        ...
        ':source_mode' => $data['created_from'] ?? 'create'
    ]);

    $comparisonId = $pdo->lastInsertId();

    // 2. INSERT Plan Rows (Comparison_Plan_Row)
    if (!empty($data['plan_rows']) && is_array($data['plan_rows'])) {
        ...
        foreach ($data['plan_rows'] as $index => $row) {
            ...
            // Clamp gap_percent sebelum insert
            $gapPercent = clampGapPercent($row['gap_percent'] ?? 0);
            ...
            $planStmt->execute([
                ...
                ':gap_percent' => $gapPercent, // Sudah di-clamp
